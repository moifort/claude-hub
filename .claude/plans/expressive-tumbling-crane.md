# Plan : Création du CLAUDE.md

## Context

Le projet ClaudeHub n'a aucun CLAUDE.md. Il faut en créer un pour guider les futures instances de Claude Code dans ce repo. L'utilisateur demande aussi d'inclure la référence au projet `../liquid-glass-reference` et les skills Swift pertinents.

## Action

Créer `/Users/thibaut/Code/claude-hub/CLAUDE.md` avec le contenu suivant :
...
1. `docs: add CLAUDE.md with project guidance for Claude Code`

## Verification

- Verify the file is well-formed markdown
- Ensure no duplication with global `~/.claude/CLAUDE.md` instructions
